...
/**
 * Raymarched gradient wave field (WebGL2, ogl). Runs only where it earns its
 * cost: paused offscreen or when the tab is hidden, quality drops on small
 * screens, and it renders nothing at all under reduced motion / no WebGL2.
 */
...
export function GradientWaves({
  ...
  useEffect(() => {
    ...
    const tier: Detail = small ? "low" : p.detail;
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSteps: { value: detailToSteps(tier) },
        uBrightness: { value: p.brightness },
        uOpacity: { value: p.opacity },
        uGrain: { value: p.grain ? 1 : 0 },
        uGrainIntensity: { value: p.grainIntensity },
        uMouse: { value: new Float32Array([0.5, 0.5]) },
        uParallax: { value: p.parallaxStrength },
        uEnableMouse: { value: p.mouseInteraction && !small },
